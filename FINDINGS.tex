...
loss. They y axis represents the ``feature density'' of a model's training data
(that is 1 - sparsity).
...
Another thing I did to make training more efficient was to initalize the weights and
...
\begin{figure}[h]
    ...
    \captionsetup{font=footnotesize, width=0.7\linewidth} % Set the font size for this caption
    \caption{
        Loss for ReLU models trained in this replication. Note that the each point
...
each illustrate
analogous overall trends. When a one-nueron ReLU model is trained with high feature
...
only the first input interally ($W \approx [1, 0]$). If the second feature has 
...
There is, however, also clearly a few important differences between the graphs in
...
be partially because of slighly different methods used in interpreting the results
of training these one-neuron models. In the original paper, the authors train
10 batchs of real models saying they ``average over the results, discarding 
...
weather the weight matrix $W$ represented only the first feature or only the second feauture.
...
section, I only trained each linear model once. This proved to be enought to get
a general idea of how the models behave in practice. \\

In theory these linear models should not exibit superposition. It is in the best
...
these linear models do not always do exactly what they ``should'' do. Although were trained
extensively---6,000 batchs of 128 examples---they typically did not do what would
...
what is most advantagous for it, it is more likly to do the things it
is encentivized to do. In theory this graph should be all orange and purple. In
...
Models of Superposition}: in practice it can be fairly complex to predict weather
a model should represent a feature in superposition.

\section{Conclusion}

This replication demostrates that it is possible for models to represent 
...
hand, if one feature is exceedingly important is is very unlikely that that
...
batch size. Thus, I beleive that it is wise to be cautious when making broad 
...
are a number of important factors that influence weather or not a model with 
